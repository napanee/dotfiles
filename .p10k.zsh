# Generated by Powerlevel10k configuration wizard on 2026-07-22 at 10:39 CEST.
...
# vertical separators, flat heads, flat tails, 2 lines, solid, left frame, sparse,
...
() {
  ...
  # Empty means never shorten the directory based on absolute length.
  typeset -g POWERLEVEL9K_DIR_MAX_LENGTH=
  # When `dir` segment is on the last prompt line, try to shorten it enough to leave at least this
  # many columns for typing commands. 0 disables this, so the path is never shortened for it.
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      # If the branch name contains a ticket prefix (e.g. feature/ECOMPR-305_foo),
      # truncate it to just that prefix (-> feature/ECOMPR-305).
      # Otherwise fall back to the middle-truncation for names longer than 32 chars.
      # Tip: To always show local branch name in full without truncation, delete the next block.
      if [[ $branch =~ '^(.*/[[:alpha:]]+-[0-9]+)' ]]; then
        branch="${match[1]} …"
      elif (( $#branch > 32 )); then
        branch[13,-13]="…"
      fi
